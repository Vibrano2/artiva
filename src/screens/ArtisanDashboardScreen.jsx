...
import { Lock, DollarSign, Award, Star, ShieldCheck, Power, MessageSquare, FileText, MapPin } from 'lucide-react';
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-card flex items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-1.5">
              ...
              <span className="p-0.5 bg-[#16858F] text-white rounded-full" title="Profile approved">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-amber-50 p-4 rounded-2xl border border-amber-200/80 shadow-sm space-y-1">
              ...
              <p className="text-xl font-extrabold text-amber-950 font-['Outfit']">
                ₦{Number(stats?.held_total ?? 0).toLocaleString()}
              </p>
            ...
            <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-200/80 shadow-sm space-y-1">
              ...
              <p className="text-xl font-extrabold text-emerald-950 font-['Outfit']">
                ₦{Number(stats?.released_total ?? 0).toLocaleString()}
              </p>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm space-y-1">
              ...
              <p className="text-xl font-extrabold text-[#0E3B40] font-['Outfit']">
                {stats?.reputation_score == null ? 'Not rated' : `${stats.reputation_score} ★`}
              </p>
                ...
                >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-2 py-0.5 text-[10px] font-extrabold rounded-full ${
                          j.status === 'completed' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'
                        }`}>
                          {j.status}
                        </span>
                      </div>
                  ...
                  {j.status === 'in_progress' && (
                    <div className="grid grid-cols-3 gap-2 pt-1 border-t border-slate-100">
                      ...
                      <button
                        onClick={() => navigateTo('live_tracking', { job: j })}
                        className="py-2 bg-slate-100 text-slate-700 text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 hover:bg-slate-200 transition-colors"
                      >
